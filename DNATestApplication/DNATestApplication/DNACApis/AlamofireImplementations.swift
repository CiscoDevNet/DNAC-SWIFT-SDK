...
class SecurityCertificateManager {
    ...
    let defaultManager: Alamofire.SessionManager = {
        ...
        let serverTrustPolicies: [String: ServerTrustPolicy] = [
            DNAAuthenticationManager.shared.getDNAC_IP(): .disableEvaluation
        ]
...
open class AlamofireRequestBuilder<T>: RequestBuilder<T> {
    ...
    open func createSessionManager() -> Alamofire.SessionManager {
       ...
       // if DNAAuthenticationManager.shared.shouldForceTrust == true{
            return SecurityCertificateManager.sharedInstance.defaultManager
       // }
        ...
        default:
            validatedRequest.responseJSON(options: .allowFragments) { response in
                ...
                if let json: Any = response.result.value {
                    ...
                    case let .failure(error): completion(nil, ErrorResponse.DecodeError(response: response.data, decodsseError: error))
                    }
                    return
                } else if "" is T {
                    // parser currently doesn't support void, which will be fixed in future swagger-parser release
                    completion(Response(response: response.response!, body: ("" as! T)), nil)
                    return
                }
